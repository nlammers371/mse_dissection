...
%             trace
    ...
    auto_samples = NaN(lags+1,n_boots);    
    ...
    for b = 1:n_boots
        ...
        if bootstrap
            s_vec = 1:size(auto_array,2);
            s = randsample(s_vec,length(s_vec),true,trace_weights);
            auto_sample = auto_array(:,s);
            time_sample = time_steps(:,s);
        else
            auto_sample = traces;
        end       
        %Take weighted mean. Traces with length < lags should just not be
        %factored in for lags beyond their length
        numerator = sum(auto_sample.*time_sample,2);
        denominator = sum(time_sample,2);
        auto_samples(:,b) = numerator ./ denominator;
        dd_samples(:,b) = diff(diff(auto_samples(:,b)));
    end
    wt_autocorr = mean(auto_samples,2);
    wt_dd = mean(dd_samples,2);
    a_boot_errors = std(auto_samples,[],2);
    dd_boot_errors = std(dd_samples,[],2);